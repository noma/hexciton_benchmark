...
if [ "$BUILT_CPU" = "true" ]
then
	build "cpu" "-DNUM_ITERATIONS=${NUM_ITERATIONS} -DNUM_WARMUP=${NUM_WARMUP} -DINTEL_PREFETCH_LEVEL=${INTEL_PREFETCH_LEVEL} -DVEC_LENGTH=4 -DVEC_LENGTH_AUTO=16 -DBUILD_OPTIONS=${BUILD_OPTIONS}"
	BUILT_SOMETHING=true
fi

if [ "$BUILT_KNL" = "true" ]
then
	build "knl" "-DNUM_ITERATIONS=${NUM_ITERATIONS} -DNUM_WARMUP=${NUM_WARMUP} -DINTEL_PREFETCH_LEVEL=${INTEL_PREFETCH_LEVEL} -DVEC_LENGTH=8 -DVEC_LENGTH_AUTO=16 -DBUILD_OPTIONS=${BUILD_OPTIONS}"
	BUILT_SOMETHING=true
fi

if [ "$BUILT_KNC" = "true" ]
then
	build "knc" "-DNUM_ITERATIONS=${NUM_ITERATIONS} -DNUM_WARMUP=${NUM_WARMUP} -DINTEL_PREFETCH_LEVEL=${INTEL_PREFETCH_LEVEL} -DVEC_LENGTH=8 -DVEC_LENGTH_AUTO=16 -DBUILD_OPTIONS=${BUILD_OPTIONS}"
	BUILT_SOMETHING=true
fi

if [ "$BUILT_GPU" = "true" ]
then
	build "gpu" "-DNUM_ITERATIONS=${NUM_ITERATIONS} -DNUM_WARMUP=${NUM_WARMUP} -DVEC_LENGTH=8 -DVEC_LENGTH_AUTO=16 -DBUILD_OPTIONS=${BUILD_OPTIONS}"
	BUILT_SOMETHING=true
fi
