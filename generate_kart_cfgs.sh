...
for compiler in $COMPILERS; do
	...
	case $compiler in
		g++)
			...
			;;
		clang++)
			# find g++ installation from PATH
			GCC_BIN=$(dirname $(which g++))
			if [[ ! -z $GCC_BIN ]]
			then
				GCC_DIR=$GCC_BIN/..
				GCC_TOOLCHAIN="--gcc-toolchain=$GCC_DIR"
				COMPILER_OPTIONS_ALWAYS_CLANG="$COMPILER_OPTIONS_ALWAYS_CLANG $GCC_TOOLCHAIN"
				LINKER_OPTIONS_ALWAYS_CLANG="$LINKER_OPTIONS_ALWAYS_CLANG $GCC_TOOLCHAIN"
			fi
